...
2. **入力・表示（Preview）の安定化**（ユーザー報告: 「ヘッダーが入力しても表示に反映されない」問題の再現と修正）。
3. 既存UI変更後の回帰確認（Home/Auth/Note編集/BottomSheet/SuggestBar/Toast）。

## Doneの定義
- 見出し入力（例: `# H1`）が編集エリア・プレビュー双方で意図どおり表示される。
...
- 最新作業コミット（現在HEAD）: `ec8630b`（メッセージ: `Codex-generated pull request`）。
- 上記コミットでUIのミニマル化、英語文言統一、いくつかのリファクタリングを実施。
- 変更対象は以下9ファイル（詳細はセクション3）。
- 現セッションでは**機能改善は未実施**。引き継ぎ資料作成のみ実施。

> 重要: ユーザーから「前回成果に不満足」と明示あり。とくに **Obsidian機能再現** と **入力/プレビュー安定化** を次セッションの主タスクに設定すること。
...
## 不明（要確認）
- ユーザー報告の「ヘッダー入力が表示に反映されない」現象を、**有効なSupabase環境で再現確認できていない**。
